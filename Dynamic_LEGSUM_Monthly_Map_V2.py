...
if uploaded_legsum_file and uploaded_tlorder_driverpay_file and uploaded_isaac_owner_ops_file and uploaded_isaac_company_trucks_file:
    ...
    with st.expander("Table and Map for Power Unit"):
        ...
        if filtered_view.empty:
            ...
            # --- CRUCIAL LINES (reiterated for clarity) ---
            filtered_view['Profit (CAD)'] = filtered_view['TOTAL_CHARGE_CAD'] - filtered_view['Driver Pay (CAD)']
            filtered_view['Revenue per Mile'] = np.where(
                pd.notna(filtered_view['Bill Distance (miles)']) & (filtered_view['Bill Distance (miles)'] > 0),
                filtered_view['TOTAL_CHARGE_CAD'] / filtered_view['Bill Distance (miles)'],
            ...
            # Identify if each power unit is an Owner Operator and add lease cost
            owner_ops_units = set(owner_ops_fuel_df['VEHICLE_NO'])  # Get unique power units in the Owner Ops report
            merged_df['Is Owner Operator'] = merged_df['LS_POWER_UNIT'].isin(owner_ops_units)  # True if in Owner Ops
            ...
            # Add Fuel Cost column to route summary
            route_summary_df['Fuel Cost'] = merged_df['Fuel Cost']

    with st.expander("All Grand Totals"):
        if 'route_summary_df' in locals() and not route_summary_df.empty:
            grand_totals_df = route_summary_df.copy()

            # Fix: Handle empty strings and convert to numeric
            for col in ["Total Charge (CAD)", "Driver Pay (CAD)", "Profit (CAD)", "Bill Distance (miles)", "Leg Distance (miles)", "Lease Cost", "Fuel Cost"]:
                grand_totals_df[col] = grand_totals_df[col].astype(str).str.replace(r'[$,]', '', regex=True)
                grand_totals_df[col] = grand_totals_df[col].replace('', np.nan)
                grand_totals_df[col] = pd.to_numeric(grand_totals_df[col], errors='coerce')
            ...
            for col in ["Total Charge (CAD)", "Driver Pay (CAD)", "Profit (CAD)", "Lease Cost", "Fuel Cost"]:
