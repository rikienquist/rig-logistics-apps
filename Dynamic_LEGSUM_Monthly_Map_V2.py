...
if uploaded_legsum_file and uploaded_tlorder_driverpay_file and uploaded_isaac_owner_ops_file and uploaded_isaac_company_trucks_file:
    with st.expander("All Grand Totals"):
        ...
        # Remove conflicting duplicates: retain only one row per unique combination of identifiers
        valid_rows = valid_rows.sort_values(by=['LS_ACTUAL_DATE', 'TOTAL_CHARGE_CAD']).drop_duplicates(subset=[
            'LS_POWER_UNIT', 'BILL_NUMBER', 'LS_ACTUAL_DATE', 'TOTAL_CHARGE_CAD'
        ], keep='last')

        # Debugging: Validate rows contributing to Power Unit 471 calculations
        unit_471_rows = valid_rows[valid_rows['LS_POWER_UNIT'] == '471']
        st.write("Rows Contributing to Unit 471 After Removing Conflicting Duplicates (Debugging):")
        ...
        st.write("All Grand Totals After Removing Conflicting Duplicates (Debugging):")
