...
def preprocess_legsum(file, location_coords):
    ...
    location_coords['LOCATION'] = location_coords['LOCATION'].str.upper()

    df['LEGO_ZONE_DESC'] = df['LEGO_ZONE_DESC'].str.upper()
    df['LEGD_ZONE_DESC'] = df['LEGD_ZONE_DESC'].str.upper()

    # Merge for LEGO_ZONE_DESC and LEGD_ZONE_DESC
...
def preprocess_tlorder(file):
    df = pd.read_csv(file, low_memory=False)
    df.rename(columns={
        'BILL_NUMBER': 'TLORDER_BILL_NUMBER'
    }, inplace=True)
    return df
...
def preprocess_driverpay(file):
    df = pd.read_csv(file, low_memory=False)
    df.rename(columns={
        'BILL_NUMBER': 'DRIVERPAY_BILL_NUMBER'
    }, inplace=True)
    df['TOTAL_PAY_AMT'] = pd.to_numeric(df['TOTAL_PAY_AMT'], errors='coerce')
    driver_pay_agg = df.groupby('DRIVERPAY_BILL_NUMBER').agg({
        'TOTAL_PAY_AMT': 'sum',
        'DRIVER_ID': 'first'
    }).reset_index()
...
if uploaded_legsum_file and uploaded_tlorder_file and uploaded_driverpay_file:
    # Load city (location) coordinates
    city_coordinates_df = load_location_coordinates()
    legsum_df = preprocess_legsum(uploaded_legsum_file, city_coordinates_df)

    # Preprocess TLORDER and merge with LEGSUM
    ...
    # Preprocess DRIVERPAY and merge with LEGSUM
    driver_pay_agg = preprocess_driverpay(uploaded_driverpay_file)
    legsum_df = legsum_df.merge(driver_pay_agg, left_on='LS_FREIGHT', right_on='DRIVERPAY_BILL_NUMBER', how='left')
    ...
    # Calculate TOTAL_CHARGE_CAD only for rows where a BILL_NUMBER exists
    legsum_df['TOTAL_CHARGE_CAD'] = np.where(
        pd.notna(legsum_df['TLORDER_BILL_NUMBER']),  # Check if BILL_NUMBER exists
    ...
    # Calculate Revenue per Mile only if LS_LEG_DIST > 0
    ...
    # Filter data based on selected Power Unit and Driver
    ...
    # Filter data based on available months
    ...
    if not month_data.empty:
        ...
        # Track sequence of location appearances
        ...
        # Legend tracking to avoid duplicate entries
        legend_added = {"Origin": False, "Destination": False, "Route": False}
        
        # Iterate through each row to build map traces
        for _, row in month_data.iterrows():
            ...
            # Get aggregated values for the origin location
            ...
            # Get aggregated values for the destination location
        ...
        if not relevant_missing_locations.empty:
            ...
            st.write("All locations have valid coordinates.")

else:
    st.error("Please upload LEGSUM, TLORDER, and DRIVERPAY CSV files to proceed.")
